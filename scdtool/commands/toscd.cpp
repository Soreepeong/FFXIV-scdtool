...
int cmd_toscd(const std::vector<std::string>& args) {
	argparse::ArgumentParser parser("scdtool toscd");
	try {
		parser
			.add_description("Create a single-entry SCD file.")
			.add_epilog("\n"
				R"(Usage examples:)" "\n"
				R"(* scdtool toscd -t "C:\Program Files (x86)\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game::music/ex2/BGM_EX2_System_Title.scd")" "\n"
		...
		parser
			...
			// 0u, not 0: argparse stores the default as std::any and the action below
			// returns uint32_t, so an `int` default makes get<uint32_t> throw bad_any_cast.
			.default_value(0u)
			.required()
	...
	try {
		...
		if (!newEntryFilled) {
			...
			// Fills the newEntry declared above; do not declare a copy here, or the
			// encoded entry below is discarded and an empty one is written instead.
